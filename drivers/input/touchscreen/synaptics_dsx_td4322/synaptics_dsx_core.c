...
/* virtual touch create date */
#define MAX_VIRTOUCH_TOUCH 	0x20
#define VBUTTON_DOWN		1
#define VBUTTON_UP			0
#define VIR_SOURCE			0
extern struct semaphore virtual_touch_sem;
extern int virtual_touch_set_dev(struct input_dev *dev);
extern int slot_conver_mechanism(void *identity, int id, int state, int source);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	retval = down_interruptible(&virtual_touch_sem);
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			if (stylus_presence)
				break;
#ifdef TYPE_B_PROTOCOL
			// slot = slot_conver_mechanism(rmi4_data->input_dev,
			// 		finger, VBUTTON_DOWN, VIR_SOURCE);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	/*  20220106 if screen is sleep, no touch event, start. */
	unsigned char buf[16];
	struct file *fp;
	mm_segment_t fs;
	loff_t pos;
	fp = filp_open("/sys/class/gpio5_pwm2/en_backlight", O_RDWR, 0644);
	if (IS_ERR(fp)) {
		TP_LOGD("[syn] create file error.###\n");
	} else {
		fs = get_fs();
		set_fs(KERNEL_DS);
		pos = 0;
		...
		TP_LOGD("[syn]read /sys/class/gpio5_pwm2/en_backlight, value=%d.\n", buf[0]);
		filp_close(fp, NULL);
		set_fs(fs);
		if(buf[0]==0x30){ // 0x30 = '0'
			TP_LOGD("[syn] /sys/class/gpio5_pwm2/en_backlight=0.\n");
			return;
		}
	}
	/*  20220106 if screen is sleep, no touch event, end. */
...
static void synaptics_rmi4_sensor_report_workqueue(struct work_struct *work)
{
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	queue_delayed_work(rmi4_data->report_workqueue,
				&rmi4_data->report_work, 0);
...
static void synaptics_rmi4_set_params(struct synaptics_rmi4_data *rmi4_data)
{
	...
	input_mt_init_slots(rmi4_data->input_dev,
		MAX_VIRTOUCH_TOUCH, INPUT_MT_DIRECT);
#else
	input_mt_init_slots(rmi4_data->input_dev,
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	virtual_touch_set_dev(rmi4_data->input_dev);
		...
		bool enable)
{
	...
	if (rmi4_data->bus_reg) {
		...
		printk("[kevin]regulator_enable bus_reg\n");
		if (retval < 0) {
	...
	//	printk("[kevin]regulator_enable pwr_reg\n");
	//	if (retval < 0) {
...
disable_pwr_reg:
	if (rmi4_data->pwr_reg)
		...
		//regulator_force_disable(rmi4_data->pwr_reg);
		printk("[kevin]regulator_disable pwr_reg\n");

//disable_bus_reg:
//	if (rmi4_data->bus_reg)
//		regulator_disable(rmi4_data->bus_reg);
//		//regulator_force_disable(rmi4_data->bus_reg);
//		printk("[kevin]regulator_disable bus_reg\n");
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_FB
	rmi4_data->fb_notifier.notifier_call = synaptics_rmi4_dsi_panel_notifier_cb;//synaptics_rmi4_fb_notifier_cb;
	//retval = fb_register_client(&rmi4_data->fb_notifier);
	msm_drm_register_client(&rmi4_data->fb_notifier);
	printk("[kevin]fb_register_client %d\n",retval);
	if (retval < 0) {
	...
	rmi4_data->report_workqueue = create_singlethread_workqueue("rmi4 sensor report workqueue");
	INIT_DELAYED_WORK(&rmi4_data->report_work, synaptics_rmi4_sensor_report_workqueue);
...
#endif
	cancel_delayed_work_sync(&rmi4_data->report_work);
		...
		unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && rmi4_data) {
		...
		printk("[kevin]%s-%d,transition = %d,event = %d\n",__func__,__LINE__,*transition,event);
		if ((event == MSM_DRM_EARLY_EVENT_BLANK) && (rmi4_data->fb_ready == false)) {
			/* Resume */
		...
		} else if ((event == MSM_DRM_EVENT_BLANK)&& (rmi4_data->fb_ready == true)) {
			/* Suspend */
		...
		unsigned long event, void *data)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			...
			fb_notifier);
	/*if(evdata!=NULL)
		printk("[WUFEI] event = %ld\n", event);
	if(evdata->info!=NULL)
		printk("[WUFEI] evdata->info->count=%d\n", evdata->info->count);
 	if(evdata->data!=NULL)
		printk("[WUFEI] evdata->data=%s\n", evdata->data);*/
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	printk("[kevin]synaptics suspend +++\n");
	...
	// if (!rmi4_data->suspend) {
	// 	synaptics_rmi4_irq_enable(rmi4_data, false, false);
	// 	// if (dp_display_connected == DP_PULL_OUT){
	// 	// 	printk("DP display is pull out, do nothing!\n");
	// 	// }else{
	// 	// 	//printk("DP display is free, disabled irq!\n");
	// 	// 	synaptics_rmi4_irq_enable(rmi4_data, false, false);
	// 	// 	dp_display_connected = DP_UNUSED;
	// 	// }
	// 	//synaptics_rmi4_sleep_enable(rmi4_data, true);
